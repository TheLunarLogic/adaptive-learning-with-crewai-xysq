...
def upload_document(credentials: dict, content: bytes, filename: str, mime_type: str) -> str:
    ...
    try:
        ...
        # Check if file already exists in xysq
        try:
            existing = client.organise.list_files(folder_id=folder_id)
            if any(getattr(f, "name", getattr(f, "filename", None)) == filename for f in existing):
                return f"✓ {filename} already exists in persistent learning memory"
        except Exception:
...
def get_document_context(credentials: dict | None, topic: str) -> str:
    """Retrieve extracted content of an uploaded document for *topic* from xysq Organise.

    Searches the learning-materials folder for any file whose name matches
    the topic (case-insensitive, ignoring separators).  Returns the extracted
    text so agents can treat it as high-priority source material.

    Returns an empty string when no matching document is found or on any error.
    """
    if not credentials:
        return ""
    try:
        client = _client(credentials["XYSQ_API_KEY"])
        folder_id = _ensure_folder(client)
        files = client.organise.list_files(folder_id=folder_id)
    except Exception as exc:
        logging.getLogger(__name__).warning("organise.list_files() failed: %s", exc)
        return ""

    # Normalise the topic for fuzzy filename matching
    topic_key = re.sub(r"[\s\-_]+", "", topic).lower()

    for f in files:
        raw_name = getattr(f, "name", None) or getattr(f, "filename", None) or ""
        # Strip extension and normalise
        name_key = re.sub(r"[\s\-_]+", "", Path(raw_name).stem).lower()
        if topic_key in name_key or name_key in topic_key:
            # Try to get the asset_id / file_id for content retrieval
            asset_id = getattr(f, "asset_id", None) or getattr(f, "id", None)
            if not asset_id:
                continue
            try:
                content = client.organise.get_file_content(asset_id)
                text = getattr(content, "text", None) or getattr(content, "content", None) or ""
                ...
                logging.getLogger(__name__).warning(
                    "organise.get_file_content(%s) failed: %s", asset_id, exc
                )
    return ""
